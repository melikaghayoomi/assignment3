...
centralization.betweenness(g, directed = TRUE, nobigint = TRUE, normalized = TRUE)
plot(g,layout=layout.fruchterman.reingold, 
     vertex.frame.color="gray",vertex.label.color="black", edge.arrow.size = 0.4, vertex.dist = 2, vertex.size = 15)
...
```{r}
EDGENEW <- EDGE %>% group_by(to) %>% mutate(received.sum = sum(count))

g <- graph.data.frame(D3, directed = TRUE, vertices=VERTEX)
plot(g, layout=layout.fruchterman.reingold, vertex.size = D3$received.sum*3, edge.arrow.size = 0.3, edge.color = "black", vertex.color = VERTEX$major, edge.width = 2)
```
